...
** Background
GitHub uses [[https://github.com/wallyqs/org-ruby][org-ruby]] to parse an org document and export it as HTML, then it is sanitized using their [[https://github.com/gjtorikian/html-pipeline/blob/main/lib/html/pipeline/sanitization_filter.rb][HTML-pipeline]].
...
The problem with this, is that the translations used by GitHub/org-ruby are incomplete. This is can be fixed by including a YAML file, but we don't need another file in our repository just for formatting a README.
...
However, the following do not work correctly and must be fixed by using HTML:
**** Centering
Org-mode's =#+BEGIN_CENTER= and =#+END_CENTER= syntax are translated to =<div>= which is missing it's ~align="center"~ attribute.
...
**** Folding
In general this is missing from org-mode, a feature markdown has that "we" don't.
